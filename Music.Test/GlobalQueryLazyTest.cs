...
namespace Music.Test
{
    ...
    public class GlobalQueryLazyTest
    {
        ...
        static ResetLazy<List<string>> albumNames = CacheLazy.Create(() => Database.Query<AlbumDN>().Select(a => a.Name).ToList());
        static ResetLazy<List<Tuple<string, string>>> albumLabelNames = CacheLazy.Create(() => (from a in Database.Query<AlbumDN>().Where(a => a.Id > 1)
        ...
        {
            var cs = ((SqlConnector)Connector.Current).ConnectionString;
